...
bool load_config(const char *filename, aiconfiguration_t *ctx) {
    return ini_parse(filename, handler, ctx);
}
...
void parse_int_string_to_array(const char *value, uint16_t *arr, uint32_t max_count);

uint32_t count_lines_in_file(const char *filepath);
...
int handler(void *data, const char *section, const char *name, const char *value) {
    ...
    if (strcmp_ignorecase(section, "dataset") == 0) {
        if (strcmp_ignorecase(name, "input") == 0) {
            parse_int_string_to_array(value, ctx->input_shape, 4);
        } else {
            return 0;
        }
        return 1;
    ...
    if (strncmp(section, "layer", 5) == 0) {
        ...
        if (strcmp_ignorecase(name, "type") == 0) {
...
Loss parse_loss(const char *value) {
    ...
    if (strcmp(value, "crossentropy") == 0) return CROSSENTROPY;
    return UNKNOWN_LOSS;
}
